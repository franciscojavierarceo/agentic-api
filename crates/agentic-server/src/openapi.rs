...
#[openapi(
    ...
    components(schemas(
        ...
        agentic_core::types::io::ShellCallOutcome,
...
mod tests {
    ...
    fn serialized_fixtures_validate_against_component_schemas() {
        ...
        let input_items = vec![
            ...
            serde_json::json!({"type": "shell_call", "id": "sh_1", "call_id": "c1", "action": {"commands": ["pwd"], "timeout_ms": 1000}, "status": "completed"}),
            serde_json::json!({"type": "custom_tool_call_output", "call_id": "c1", "output": "r"}),
            serde_json::json!({"type": "shell_call_output", "call_id": "c1", "output": [{"stdout": "ok", "stderr": "", "outcome": {"type": "exit", "exit_code": 0}}, {"outcome": {"type": "timeout"}}]}),
